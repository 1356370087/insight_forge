...
- **Multi-Agent 双层运行时：** 设计外层 `QueryEngine` 与可复用内层 Agent Loop。外层负责任务理解、研究调度、报告生成和记忆管理，内层通过 Hook 机制统一 Supervisor 与 Researcher 的模型调用、工具执行、停止判断和状态更新，降低多 Agent 运行逻辑的重复开发成本。

- **同步与异步 SubAgent 调度：** 构建同步并发与异步任务化两种调度模式，通过并发控制和可复用 Teammate Pool 执行独立研究任务，使主 Agent 能够持续规划、接收反馈并动态调整研究方向，提升复杂任务的并行处理能力。

- **可靠协作与中断恢复：** 构建基于文件邮箱、任务租约和 Checkpoint 的跨进程协作机制，支持消息去重、确认、超时重投和异常隔离；结合 Leader Lease、会话日志与结果复用，实现任务中断恢复，并避免阶段重复执行和重复检索。

- **渐进式搜索策略：** 设计“宽泛探索—证据反思—定向收窄”的搜索流程，先建立问题全景并识别权威来源，再围绕真实证据缺口补充约束；针对低召回结果主动放宽查询，提升复杂问题的搜索覆盖度和有效信息召回率。

- **研究压缩与质量门禁：** 设计独立研究上下文与分层证据机制，将检索过程持久化为可校验的研究档案，主 Agent 默认仅接收结构化摘要，并支持按需回读原始证据；引入独立评估模型审核工具结果和 SubAgent 交付质量，阻断低质量信息传播。

- **Docker 沙箱与网络治理：** 为异步 Researcher 构建任务级 Docker 沙箱，通过文件隔离、最小权限、资源配额和执行超时约束运行环境；设计从禁止联网到开放联网的分级网络策略，结合域名审批、目标校验与 SSRF 防护，控制 Agent 的外部访问边界。

- **提示词注入与信任边界防护：** 将网页、搜索和 MCP 内容统一视为不可信数据，通过内容清洗、注入检测和证据封装隔离外部指令与有效事实；同时限制客户端伪造高权限消息，并对长期记忆、模型上下文和最终报告实施分层过滤。

- **长期记忆与上下文管理：** 基于 Mem0 构建“观察—反思—画像”三层长期记忆，使 Agent 能够跨会话延续用户偏好、项目背景和可信研究结论；仅写入用户明确表达及多来源验证证据，融合相关性、重要性和时间衰减进行可解释召回，并通过访问强化、冲突判定、时态版本化与软遗忘避免陈旧或矛盾信息污染后续研究。

- **统一工具治理与预算控制：** 建立覆盖工具注册、角色授权、参数校验、敏感操作审批、网络出口检查和失败重试的统一执行链路；从 Agent 轮次、并发量、工具调用、搜索抓取和 Token 消耗等维度实施预算控制，提升工具调用的安全性与稳定性。

- **结构化 Web 证据与报告生成：** 构建从候选发现、去重重排、文档抓取到证据抽取的 Web 流水线，将搜索结果与可引用证据分离，仅允许有效文档进入引用；基于统一报告层支持多种生成策略、内容类型、输出格式和引用样式。

- **可观测性与运维：** 搭建覆盖任务、Agent、模型和工具调用的全链路追踪体系，统一记录耗时、Token、成本、缓存、质量评分及异常信息，并支持 Langfuse 镜像；结合 Prometheus 与 Grafana 建立运行质量、工具效率、证据覆盖和任务调度监控。

- **系统评估与质量闭环：** 打通本地评估与 LangSmith 批量评测流程，构建覆盖 6 类、13 项指标的 LLM-as-Judge 体系，并通过非补偿式门禁约束证据、引用和来源质量；支持基于事件日志独立重跑评估，实测综合得分 85.32/100，并定位证据链不足的质量瓶颈。
